...
from contract import MODEL_ENABLED_KINDS, REGISTERED_KINDS, contract_metadata, normalize_text
...
def build_examples(db_path: Path, seed: int) -> list[dict[str, Any]]:
    ...
    for index in range(520):
        ...
        style = index % 6
        if style == 0:
            user = f"عميل: {name} | هاتف: {mobile} | مشروع: {title} | كود: {product['sku']} | كمية: {quantity}" + (f" | مصنع: {supplier}" if supplier else "")
        elif style == 1:
            user = f"العميل {name} هاتف {mobile} أوردر {title} المنتج {product['sku']} عدد {quantity}" + (f" عند مصنع {supplier}" if supplier else "")
        elif style == 2:
            user = f"ضيف اوردر ل {name} موبايل {arabic_digits(mobile)} مشروع {title} كود {product['sku'].lower()} كمية {arabic_digits(str(quantity))}" + (f" مورد {supplier}" if supplier else "")
        elif style == 3:
            user = f"customer: {name} | phone: {mobile} | order: {title} | sku: {product['sku']} | qty: {quantity}" + (f" | supplier: {supplier}" if supplier else "")
        elif style == 4:
            user = f"اعمل طلب {title} للعميل {name} تليفون {mobile} بكود {product['sku']} عدد {quantity}" + (f" من مصنع {supplier}" if supplier else "")
        else:
